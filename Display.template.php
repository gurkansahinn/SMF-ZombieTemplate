...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		if(isset($message['member']['group_id']))
		{
			switch($message['member']['group_id'])
			{
				case '11':
				{
					$amper_bg = 'background-image: url(Themes/newbalance_20/images/adminbg.png);';
					break;
				}
				case '10':
				{
				...
				default:
				{
					$amper_bg = 'background-image: url(Themes/newbalance_20/images/survivorbg.png);';
				}
			}
		}
		else
		...
		$forumhesabibul = mysql_num_rows(mysql_query("SELECT * FROM sm_members WHERE real_name = '$membername'"));
		if($forumhesabibul >= 1)
		{
			$f_bul = mysql_query("SELECT * FROM sm_members WHERE real_name = '$membername'");
			$ara2 = mysql_fetch_assoc($f_bul);
			$betabasarim = $ara2['betabasarim'];
			}
			else if($forumhesabibul < 1)
		{
			$betabasarim = "-1";
		}
		$forumnick = $message['member']['id'];
		$karakterbul = mysql_num_rows(mysql_query("SELECT * FROM characters WHERE forumid = '$forumnick'"));
		if($karakterbul >= 1)
		{
			$k_bul = mysql_query("SELECT * FROM characters WHERE forumid = '$forumnick'");
			$ara2 = mysql_fetch_assoc($k_bul);
			$k_adi = $ara2['Character'];
			$k_skin = $ara2['Skin'];
			$k_seviye = $ara2['Level'];
			$k_koken = $ara2['Origin'];
			$k_gender = $ara2['Gender'];
			$k_faction = $ara2['Faction'];
			$hesapturu2 = $ara2['Admin'];
			$k_fara = mysql_query("SELECT * FROM factions WHERE factionID = '$k_faction'");
			$k_fara2 = mysql_fetch_assoc($k_fara);
			if($k_faction > -1) {
				$k_olusum = $k_fara2['factionName'];
			}
			else {
				$k_olusum = "Sivil";
			}
  ...
  else if($k_gender == 2)
  {
	  $k_gender = "Kadın";
		...
		$hesapturu = "Bağlanmamış";
		$k_adi = "Test_Test";
		$betabasarim = "100 gündür hayatta";
 echo '
 
 				<div class="roundframe2" style="'.$amper_bg.'">
						<div class="cont_colum_post"><!--Inicio de columnas -->	 
				
				<div class="amper_karakter">																				 
										<div id="amper_bilgiler">
							<ul>
								<li><span style=" font-size: 14px; color: #000; " class="fa fa-user"></span> <h7>',$k_adi,'</h7></span></li>
                                <div class="amper_basarim"><img data-toggle="tooltip" data-placement="top" title="Beta Başarımı" src="/forum/Themes/newbalance_20/images/madalyalar/',$betabasarim,'.png"/></div>												
												<div id="amper_olusum"><li><h7>',$k_olusum,'</h7></li>	
                                                <div style=" margin-top: -10px; margin-left: 257px; " id="amper_level1"><b><img data-toggle="tooltip" data-placement="top" title="Seviye" src="/forum/Themes/newbalance_20/images/levellar/level-',$k_seviye,'.png"/></b></div>													
												<div id="amper_cinsiyet"><li><img data-toggle="tooltip" data-placement="top" title="',$k_gender,'" src="/forum/Themes/newbalance_20/images/cinsiyet/',$k_gender,'.png" width="16" height="16"/></li></div>	
									 ...
									 <div class="amper_skin"><img src="/forum/Themes/newbalance_20/images/skin/93.png" width="90" height="90"/></div>
					';
      ...
      echo '          <div class="left_colum"> <!--Inicio segunda columna-->
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
				echo '';
			echo '
								<div class="amper_rank">', $message['member']['group_stars'], '</div>';

        echo '          </ul>
                  </div></div>
                  <div class="right_colum">
                  <div class="amper_infop"><ul class="reset smalltext">';
...
// Show the profile, website, email address, and personal message buttons.
		...
		echo '
            ...
            <div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg2' : 'windowbg2') : 'approvebg', '">
               <div class="post_wrapper">            
                  <div class="postarea">
							<div class="flow_hidden">
								<div class="amper_keyinfo">
